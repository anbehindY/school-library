  ...
  def save_people
    ...
    @people.map do |data|
      if data.instance_of?(Student)
       update_people<< {type:"Student", name: data.name, age: data.age, id: data.id }
      else
    update_people<< {type:"Teacher", name: data.name,specialization:data.specialization, age: data.age,id: data.id}
      end
